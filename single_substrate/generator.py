"""
Eidetic Memory FDM — Hermes-3-Llama-3.1-8B Adaptation

Identical FDM encoding, curriculum, reasoning logic, and evaluation
as the Qwen3 version. Only model-specific bits change:
  - Model: NousResearch/Hermes-3-Llama-3.1-8B (32 layers, GQA, RoPE)
  - Vocab: 128,256 tokens (Llama 3.1 tokenizer)
  - Token mapping: adjusted for Llama 3.1 vocab size
  - Dtype: bfloat16
  - Batch size: 2 (8B model; gradient accumulation x2 = effective batch 4)
  - Gradient checkpointing: enabled

Architecture differences from Qwen3-0.6B:
  - 8B params vs 600M
  - 32 layers vs 28 layers
  - Llama 3.1 tokenizer (128K vocab) vs Qwen3 tokenizer (152K vocab)
  - ChatML instruct format (ignored — we train on raw sequences)

Usage:
  python eidetic_hermes3_fdm_e2e.py generate
  python eidetic_hermes3_fdm_e2e.py train
  python eidetic_hermes3_fdm_e2e.py eval [stage]
"""
...
# ============================================================
# Model configuration — ONLY SECTION THAT CHANGES vs Qwen3
# ============================================================
...
BATCH_SIZE = 2        # 8B model on 32GB; effective batch=4 via grad accumulation
GRAD_ACCUM_STEPS = 2
MAX_LENGTH = 2048
OUTPUT_PREFIX = "fdm_40ch_turbo_v3_hermes3"
CHECKPOINT_DIR = "checkpoints_fdm_40ch_turbo_v3_hermes3"
FINAL_MODEL_DIR = "fdm_40ch_turbo_v3_hermes3_model_final"


# ============================================================
# Channel definitions (IDENTICAL to all other versions)
# ============================================================
...
# ============================================================
# Reasoning logic (IDENTICAL to all other versions)
# ============================================================
...
# ============================================================
# FDM Encoder — token mapping uses Llama 3.1 vocab size
# ============================================================
...
# ============================================================
# Tokenizer expansion check — run before training to confirm
# signal stays within 512 tokens under Llama 3.1 tokenizer
# ============================================================

def check_tokenizer_expansion():
    """
    Verify that a 512-token FDM signal (256+256) doesn't expand
    beyond MAX_LENGTH under the Llama 3.1 tokenizer.
    Reports expansion ratio so it can be documented in the paper.
    """
    print("Checking tokenizer expansion for Llama 3.1...")
    ...
    for _ in range(100):
        ...
        input_text = f"[MEMORY]{fdm_text}[/MEMORY]\nQuestion: Should ALICE proceed with the mission?\nAnswer:"
        encoded_len = len(tokenizer.encode(input_text))
        lengths.append(encoded_len)

    print(f"  FDM signal token IDs: 512 (256+256)")
    print(f"  Full input after tokenization:")
    print(f"    min={min(lengths)}, max={max(lengths)}, mean={sum(lengths)/len(lengths):.1f}")
    if max(lengths) > MAX_LENGTH:
        print(f"  WARNING: max {max(lengths)} exceeds MAX_LENGTH={MAX_LENGTH}. Truncation will occur.")
    else:
        print(f"  OK: all samples fit within MAX_LENGTH={MAX_LENGTH}.")
...
STAGES = [
    {"name": "Stage 0: Easy",     "num_tokens": 256, "a_high": 1.0, "a_low": 0.0,  "num_levels": 64, "samples": 15000, "epochs": 5,  "lr": 5e-5},
    {"name": "Stage 1: Standard", "num_tokens": 256, "a_high": 1.0, "a_low": 0.1,  "num_levels": 64, "samples": 20000, "epochs": 5,  "lr": 3e-5},
    {"name": "Stage 2: Moderate", "num_tokens": 256, "a_high": 1.0, "a_low": 0.15, "num_levels": 64, "samples": 25000, "epochs": 7,  "lr": 2e-5},
    {"name": "Stage 3: Harder",   "num_tokens": 256, "a_high": 1.0, "a_low": 0.2,  "num_levels": 64, "samples": 25000, "epochs": 7,  "lr": 1e-5},
    {"name": "Stage 4: Hardest",  "num_tokens": 256, "a_high": 1.0, "a_low": 0.25, "num_levels": 64, "samples": 30000, "epochs": 10, "lr": 1e-5},
]
...
# ============================================================
# Training — gradient accumulation for 8B on 32GB
# ============================================================
...
def train():
    ...
    print(f"Batch size: {BATCH_SIZE} (grad accum steps: {GRAD_ACCUM_STEPS}, effective: {BATCH_SIZE * GRAD_ACCUM_STEPS})")
    os.makedirs(CHECKPOINT_DIR, exist_ok=True)

    tokenizer = AutoTokenizer.from_pretrained(MODEL_ID, trust_remote_code=True)

    # Llama 3.1 has an eos token but no pad token by default
    ...
    for stage_idx, stage in enumerate(STAGES):
        ...
        for epoch in range(start_epoch, stage['epochs']):
            ...
            for step, batch in enumerate(tqdm(train_loader, desc=f"S{stage_idx} E{epoch+1} Train [{MODEL_SHORT}]")):
...
# ============================================================
# Full evaluation (IDENTICAL to Qwen3 version)
# ============================================================
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print(f"Usage:")
        print(f"  python eidetic_hermes3_fdm_e2e.py check    # tokenizer expansion check")
        print(f"  python eidetic_hermes3_fdm_e2e.py generate")
        print(f"  python eidetic_hermes3_fdm_e2e.py train")
        print(f"  python eidetic_hermes3_fdm_e2e.py eval [stage]")
        sys.exit(0)

    cmd = sys.argv[1]
    if cmd == "check":
        check_tokenizer_expansion()
    elif cmd == "generate":
        generate_all_stages()
    elif cmd == "train":
        train()
