"""
Eidetic Memory FDM — LiquidAI LFM2.5-1.2B-Base Adaptation
...
as the GPT-2 Medium version. Only model-specific bits change:
  - Model: LiquidAI/LFM2.5-1.2B-Base (hybrid conv + attention)
  - Vocab: 65,536 tokens
  - Architecture: 16 blocks (10 conv + 6 GQA attention)
  - Dtype: bfloat16
  - Batch size: 2 (larger model, may need gradient accumulation)

Architecture differences from GPT-2 Medium:
  - HYBRID: 10 double-gated LIV convolution blocks + 6 GQA blocks
  - Only 6/16 blocks use attention (vs 24/24 in GPT-2)
  - Convolution blocks process LOCAL patterns, not global attention
  - If FDM decoding works here, frequency decomposition doesn't
    require full self-attention at every layer
  - 1.17B params vs 345M params
  - 16 layers vs 24 layers

NOTE on mechanistic experiments:
  - Attention ablation (Exp 2) only applies to 6 of 16 blocks
  - Causal tracing (Exp 3) works on all blocks but signature will differ
  - If model routes FDM through conv blocks, that's a finding itself

Usage:
  python eidetic_lfm2_fdm_e2e.py generate
  python eidetic_lfm2_fdm_e2e.py train
  python eidetic_lfm2_fdm_e2e.py eval [stage]

Requirements:
  transformers>=4.55.0 (for LFM2.5 support)
"""
...
MODEL_SHORT = "lfm2"
VOCAB_SIZE = 65536  # LFM2.5 vocabulary size
DTYPE = torch.bfloat16
BATCH_SIZE = 2  # Smaller batch for 1.2B model on A40
GRAD_ACCUM_STEPS = 2  # Effective batch size = 2 * 2 = 4 (matches GPT-2)
MAX_LENGTH = 1024
OUTPUT_PREFIX = "fdm_40ch_turbo_v3_lfm2"
CHECKPOINT_DIR = "checkpoints_fdm_40ch_turbo_v3_lfm2"
...
class TurboFDMSignalEncoder:
    """Identical signal processing; token IDs from LFM2.5 vocab."""
    ...
    def __init__(self, num_tokens_per_encoder=256, sample_rate=100.0,
        ...
        # Token mapping — use LFM2.5's vocab size
...
# ============================================================
# Data generation (IDENTICAL logic)
# ============================================================
...
# ============================================================
# Training — CHANGED: model loading, gradient accumulation,
...
def train():
    ...
    print(f"Architecture: HYBRID (10 conv + 6 GQA attention blocks)")
    ...
    # Gradient checkpointing — critical for 1.2B on A40
    ...
    if torch.cuda.is_available():
        ...
        print(f"VRAM: {allocated:.1f}GB / {total_mem:.1f}GB")
    ...
    for stage_idx, stage in enumerate(STAGES):
        ...
        for epoch in range(start_epoch, stage['epochs']):
            ...
            for step, batch in enumerate(tqdm(train_loader, desc=f"S{stage_idx} E{epoch+1} [{MODEL_SHORT}]")):
...
def evaluate(model_path=None, stage_idx=None):
    ...
    print(f"\n{'='*60}")
    print(f"{MODEL_SHORT.upper()} 40-CHANNEL FDM EVALUATION (HYBRID CONV+ATTENTION)")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print(f"Architecture: 10 conv blocks + 6 GQA attention blocks")
        print(f"Usage:")
        print(f"  python eidetic_lfm2_fdm_e2e.py generate")
        print(f"  python eidetic_lfm2_fdm_e2e.py train")
